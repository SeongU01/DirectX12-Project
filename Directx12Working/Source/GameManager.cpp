...
GameManager::GameManager()
{
    ...
    _pRenderer     = D12Renderer::GetInstance();
}
...
{
    _pWinApp = WinApp::Create(settingInfo.hInstance, settingInfo.appName,
                              settingInfo.width, settingInfo.height,
                              settingInfo.isFullScreen, settingInfo.showCursor);
    ...
    if (false == _pInputManager->SetUpInputDevice(settingInfo.hInstance,
                                                  _pWinApp->GetWindow()))
    {
...
void GameManager::UpdateCameraRotation(float deltaX, float deltaY,
                                       float sensitivity, const Vector4& _right)
{
